...
static BLEInterface * _sharedBLEAdapter = nil;
...
#pragma mark // Peripheral Setup, Discovery, and Connection Methods
...
- (void)beginScanForPeripheralWithServices:(NSArray *)services {
    if ([self isReadyForCommand]) {
        ...
        if ([self.delegate respondsToSelector:@selector(BLEInterfaceWillScanForPeripherals:)]) {
        ...
        self.waitingToScanForServices = services;
    }
}
...
- (void)endScanForPeripherals {
    if ([self isReadyForCommand]) {
        ...
        if ([self.delegate respondsToSelector:@selector(BLEInterfaceDidEndScanForPeripherals:)]) {
            [self.delegate BLEInterfaceDidEndScanForPeripherals:self];
        }
    } else {
        NSLog(@"Error: Unexpected condition - endScanForPeripheral request while BLEAdapter in state %d (%s)",
              (int)self.centralManager.state, [self centralManagerStateToString:self.centralManager.state]);
    }
}
...
- (void)connectToPeripheral:(CBPeripheral *)peripheral {
    if ([self isReadyForCommand]) {
        ...
        if ([self.delegate respondsToSelector:@selector(BLEInterface:willConnectToPeripheral:)]) {
            [self.delegate BLEInterface:self willConnectToPeripheral:peripheral];
        }
        
        [self.centralManager connectPeripheral:peripheral options:0];
    } else {
        NSLog(@"Could not complete connection request. Request queued. Bluetooth state: %d", (int)self.centralManager.state);
        self.waitingToConnectToPeripheral = peripheral;
    }
}
...
//    self.centralManager = [[CBCentralManager alloc] initWithDelegate:self queue:nil options:@{CBCentralManagerOptionRestoreIdentifierKey:@"BLESericeBrowser"}];
//}
 ...
 *
 *  @param services – NSArray of service CBUUIDs to search for
 *  @param timeout  - Timeout in seconds to search for BLE peripherals
 *
 *  @return 0 (Success), -1 (Fault)
 *
 *  @discussion findBLEPeripherals searches for BLE peripherals with the specified services and sets
 *              a timeout when scanning is stopped. If services is nil, peripherals with any service
 *              will be found
 *
 */
- (int)findBLEPeripheralsWithServices:(NSArray *)services timeout:(int)timeout {
    ...
    [self.centralManager scanForPeripheralsWithServices:services options:0]; // Start scanning
 ...
 *
 *  @discussion scanTimer is called when findBLEPeripherals has timed out, it stops the CentralManager
 *              from scanning further and prints out information about known peripherals
 *
 */
...
- (void)connectPeripheral:(CBPeripheral *)peripheral status:(BOOL)status {
    ...
    if(status == TRUE)
    {
        ...
        [self.centralManager connectPeripheral:activePeripheral options:nil];
    }
    else
    {
        activePeripheral = peripheral;
...
#pragma mark // Peripheral Interation Methods
